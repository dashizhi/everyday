    ...
    <div class="search-box">
        <table>
            ...
            <tr>
                <td><?=$val['id']?></td>
                <td><?=$val['fi_name']?></td>
                <?php if ($val['fi_type']=="text") { ?>
                    <td>文本框</td>
                <?php }else if ($val['fi_type']=="radio") { ?>
                    <td>单选框</td>
                <?php }else if ($val['fi_type']=="password") { ?>
                    <td>密码框</td>
                <?php }else{ ?>
                    <td>文本域</td>
                <?php }?>
                <td><?=$val['fi_val']?></td>
                <td><?=$val['is_need']==1 ? "必填":"可选" ;?></td>
                <?php if ($val['fi_rule']=="length") { ?>
                    <td>长度</td>
                <?php }else if ($val['fi_rule']=="phone") { ?>
                    <td>手机号码</td>
                <?php }?>
                <td><?=$val['fi_l_start'] ?>~<?=$val['fi_l_end']?></td>
                <td>
                    <a href="<?= Url::toRoute(['testone/add','id'=>$val['id']])?>">编辑</a>
                    |
                    <a href="<?= Url::toRoute(['testone/del','id'=>$val['id']])?>">删除</a>
                </td>
            </tr>
           <?php } ?>
        </table>
